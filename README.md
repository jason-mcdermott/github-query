...
- If you have a Github API access token, you can add to the appsettings.json file. otherwise, calls will be made 
  without bearer token (and may be throttled).
...
- Some sample endpoints to try out:
  ...
  // all ramda org / ramda repo PR's paginated and where state is closed (supports 'open', 'closed', 'all'. 'all' 
  // is the default)
...
- I leveraged existing code for the ExceptionFilter and the LinkHeaderExtensions (which saved me a lot of time and tears!). 
  Links to sources are in the code files.
